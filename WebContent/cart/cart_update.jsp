	...
	try {
		if(session.getAttribute("member_id") == null){ // 비로그인시 (세션 장바구니 처리)
			...
			if(qty == 0){
				...
				out.print("<script>alert('장바구니 정보를 수정하였습니다.');</script>");
			}
			...
			sql = "UPDATE cart SET quantity=" + qty + " WHERE cartno=" + cartno;
			cnt = stmt.executeUpdate(sql);
			
			if(cnt != 0) { // 수정 성공
				out.print("<script>alert('장바구니 정보를 수정하였습니다.'); location.href='cart_list.jsp'</script>");
			} else { // 수정 실패
				out.print("<script>alert('장바구니 정보 수정에 오류가 발생했습니다.');</script>");
			}
		}
	} catch (Exception e) {
		e.printStackTrace();
